      ...
      <Card className="mb-6 border-none shadow-none bg-transparent">
        ...
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
          <div className="flex flex-wrap gap-4 mb-4">
            <button
              className={`rounded-full text-sm font-medium  flex items-center gap-2  px-3 py-1.5 border-1 z-10 border-solid border-theme-primary-300 bg-gradient-to-b cursor-pointer transition-all ${activeTab === '1' ? 'dark:bg-theme-black-100 from-theme-purple-100 to-theme-blue-100 text-neutral-100' : 'border-transparent hover:dark:bg-theme-black-100/50'}`}
            ...
            >
              ...
              <span className={` ${activeTab === '1' ? '' : ''}`}>{t('tableDashboard.tabs.trending')}</span>
            </button>
            <button
              className={`rounded-full bg-gradient-to-b text-sm font-medium px-2 py-1 border-1 z-10 border-solid border-theme-primary-300 cursor-pointer transition-all ${activeTab === '2' ? 'dark:!bg-theme-black-100 from-theme-purple-100 to-theme-blue-100 text-neutral-100' : 'border-transparent hover:dark:bg-theme-black-100/50'}`}
              onClick={() => setActiveTab('2')}
            >
              <span className={` ${activeTab === '2' ? ' flex items-center gap-2' : 'flex items-center gap-2'}`}>{t('tableDashboard.tabs.new')} ( Pumpfun<PumpFun />)</span>
            </button>
            {isAuthenticated && (<button
              className={`rounded-full bg-gradient-to-b flex items-center gap-2 text-sm font-medium px-2 py-1 border-1 z-10 border-solid border-theme-primary-300 cursor-pointer transition-all ${activeTab === '3' ? 'dark:!bg-theme-black-100 from-theme-purple-100 to-theme-blue-100 text-neutral-100' : 'border-transparent hover:dark:bg-theme-black-100/50'}`}
              onClick={() => setActiveTab('3')}
            >
              <img src="/star-icon.png" alt="star" className="w-5 h-5" />
              <span className={` ${activeTab === '3' ? '' : ''}`}>{t('tableDashboard.tabs.favorite')}</span>
            </button>)}

            <div className="relative inline-block">
              <button
                className={`rounded-full bg-gradient-to-b text-sm font-medium px-2 py-1 border-1 z-10 border-solid flex items-center gap-2 border-theme-primary-300 cursor-pointer transition-all ${activeTab === '4' ? 'dark:!bg-theme-black-100 from-theme-purple-100 to-theme-blue-100 text-neutral-100' : 'border-transparent hover:dark:bg-theme-black-100/50'}`}
